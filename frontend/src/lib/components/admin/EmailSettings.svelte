<!--
  ...
  Organized into four subtabs: General, Signature, Persona, and Advanced.
...
<script lang="ts">
	...
	let activeTab = $state<'general' | 'signature' | 'persona' | 'advanced'>('general');

	const tabs = [
		{ id: 'general' as const, label: 'General', icon: Mail },
		{ id: 'signature' as const, label: 'Signature', icon: Signature },
		{ id: 'persona' as const, label: 'Persona', icon: CircleUser },
...
<div class="flex h-full flex-col overflow-hidden">
	...
	<div class="shrink-0 px-6 pt-6 pb-4">
		...
		<div class="flex border-b border-border">
			{#each tabs as tab}
				<button
					class="px-4 py-2 text-sm font-medium transition-colors {activeTab === tab.id ? 'border-b-2 border-accent text-accent' : 'text-text-secondary hover:text-text-primary'}"
