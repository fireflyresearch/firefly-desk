...
def _build_dev_user() -> UserSession:
    ...
    * ``FLYDESK_DEV_USER_NAME`` -- display name (default: ``"Dev Admin"``)
    * ``FLYDESK_DEV_USER_EMAIL`` -- email address (default: ``"admin@localhost"``)
    * ``FLYDESK_DEV_USER_ROLES`` -- comma-separated roles (default: ``"admin,operator"``)
    * ``FLYDESK_DEV_USER_PICTURE`` -- avatar / picture URL (default: ``None``)
    * ``FLYDESK_DEV_USER_DEPARTMENT`` -- department (default: ``None``)
    * ``FLYDESK_DEV_USER_TITLE`` -- job title (default: ``None``)
    """
    display_name = os.environ.get("FLYDESK_DEV_USER_NAME", "Dev Admin")
    email = os.environ.get("FLYDESK_DEV_USER_EMAIL", "admin@localhost")
    roles = os.environ.get("FLYDESK_DEV_USER_ROLES", "admin,operator").split(",")
    picture_url = os.environ.get("FLYDESK_DEV_USER_PICTURE") or None
    department = os.environ.get("FLYDESK_DEV_USER_DEPARTMENT") or None
