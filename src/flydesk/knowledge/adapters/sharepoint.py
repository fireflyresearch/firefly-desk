...
"""SharePoint adapter via Microsoft Graph API."""
...
class SharePointAdapter:
    """Adapter for SharePoint document libraries via Microsoft Graph API."""
    ...
    async def list_drives(self) -> list[DriveInfo]:
        try:
            ...
            client = await self._get_http_client()
            response = await client.get(f"/sites/{site_id}/drives")
            response.raise_for_status()
            data = response.json()
            return [
                DriveInfo(
            ...
            logger.exception("Failed to list SharePoint drives")
            return []
    ...
    async def list_items(
        ...
        try:
            ...
            logger.exception("Failed to list SharePoint items")
        return items
...
DocumentSourceFactory.register("sharepoint", SharePointAdapter)
